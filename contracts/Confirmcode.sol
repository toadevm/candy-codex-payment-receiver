...
contract Lollypop is ERC721, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(msg.value >= (MINT_PRICE + TX_FEE) * _mintAmount, "Insufficient payment");
        ...
        (bool sent1, ) = owner().call{value: MINT_PRICE * _mintAmount}("");
        (bool sent2, ) = FEE_RECEIVER.call{value: TX_FEE * _mintAmount}("");
        require(sent1 && sent2, "Transfer failed");

        uint256 change = msg.value - (MINT_PRICE + TX_FEE) * _mintAmount;
        if (change > 0) {
            (bool sent3, ) = msg.sender.call{value: change}("");
            require(sent3, "Change transfer failed");
        }
    }
